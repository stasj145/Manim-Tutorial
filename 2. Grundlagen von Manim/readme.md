...
Zu diesem Zweck macht es Sinn, sich erstmal einen sehr einfachen Codeteil anzusehen. Mit dem folgenden Code wird ein einfacher Kreis erzeugt und dargestellt.
...
Die grundlegende Struktur, die in dem Code zu sehen ist, ist immer die gleiche. Zuerst wird eine Klasse erzeugt, der Name dieser Klasse ist frei wählbar, sollte aber möglichst die Szene repräsentieren. In diesem Fall heisst die Szene "basic". Als nächstes wird angegeben, von welcher bestehenden Klasse die neue Klassse erben soll. In diesem Fall werden die Eigenschaften und Funktionen der Klasse ```Scene``` geerbt. Für die meisten 2D-Szenen reicht es, ```Scene``` zu wählen, es gibt aber auch andere Szenen-Klassen von denen geerbt werden kann, um verschiedene Dinge umzusetzten, die Wichtigsten sind folgende:
...
Die hier genannten Szenen stellen lediglich eine Auswahl an wichtigen Szenen dar, es gibt noch viele weitere Szenen-Klassen für verschiedene Spezialfälle. Ein vollständige Liste aller Szenen-Klassen findet sich [hier](https://docs.manim.community/en/stable/reference_index/scenes.html) in der Manim-Doku. 

Ist nun eine Klasse für die zu erzeugende Szene definiert, gilt es die Methode ```construct()``` zu überschreiben. Dazu wird, wie im Code zu sehen, einfach eine Funktion mit der Signatur ```construct(self)``` definiert. In dieser Funktion wird in der Regel der gesamte Code für das Erzeugen der Szene geschrieben.
...
In diesem Fall wird die Klasse ```Circle()``` verwendet um einen einfachen Kreis zu erzeugen. Ein erzeugtes Objekt wird allerdings nicht automatisch auch auf dem Bild angezeigt. Um Objecte dem Bild hinzuzufügen wird die Methode ```add()``` der Szenen-Klasse verwendet. Diese fügt statische Objekte dem Bild hinzu. Grundlegend ist nun eine sehr einfache Szene bereits fertig und kann gerendert werden. Da nur mit ```self.add()``` gearbeitet wurde, wird nur ein einzelnes Bild und kein Video erzeugt. Insbesondere wenn das VS-Code Plugin "Manim Sideview" verwendet wird, empfieht es sich, allerdings immer Videos zu erstellen, da "Manim Sideview" Probleme damit hat, einzelne Bilder darzustellen. Zu diesem Zweck kann am Ende noch die Methode ```wait()``` verwendet werden. Mit ```wait()``` kann ganz einfach eine Wartezeit eingefügt werden. In diesem Fall wird dadurch aus dem Bild nun ein Video mit der in  ```wait()``` angegebenen Länge.
...
## Zweite Szene
Nachdem nun eine erste eigene Szene erstellt wurde, gibt es noch einige weitere Grundlagen, die nun in dieser zweiten Szene dargestellt werden. Speziell sollen nun folgende Konzepte betrachtet werden:
...
Das nun erstellte Dreieck soll nach rechts oben bewegt werden, dazu können die Konstanten kombiniert werden. Mit der ```shift()``` Methode sieht das dann so aus:
...
Es ist natürlich auch möglich all das in eine einzelnen Zeile zu schreiben. Um dies zu zeigen sowie die Positionierung mit ```move_to()``` zu zeigen wird nun noch ein weiters Objekt erstellt. 
...
Um ein solches Text-Objekt verwenden zu können, wird eine LaTeX Installation vorrausgesetzt.
...
Wie zu sehen ist stellt Manim eine große Vielfalt an Möglichkeiten zur Verfügung, die einfach kombiniert werden können, um Szenen zu erzeugen. 
